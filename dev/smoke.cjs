...
async function run() {
  ...
  for (const marker of ['TAPEOUT', 'NEW GAME', 'CONTINUE', 'SETTINGS', 'COMBAT PROTO']) {
    assert(menuText.includes(marker), `menu missing "${marker}"`);
    checks++;
  }

  // combat prototype screen — 2D, no WebGL required
  const onProto = findClickable(root.toJSON(), 'COMBAT PROTO');
  assert(typeof onProto === 'function', 'could not find COMBAT PROTO button');
  act(() => { onProto(); });
  for (let i = 0; i < 6; i++) { await act(async () => { await Promise.resolve(); }); } // eslint-disable-line no-await-in-loop
