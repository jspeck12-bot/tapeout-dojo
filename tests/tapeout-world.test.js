...
import { TAPEOUT_PALETTE, ALTAR } from '../src/graphics/tapeout-world.js';
...
describe('Silicon Gothic TAPEOUT', () => {
  test('composes a quiet silicon cathedral with PBR, path lighting, and unshadowed point lights', () => {
    const restore = stubDocument();
    try {
      const scene = new THREE.Scene();
      const lessonIds = (LESSONS[7] || []).map(lesson => lesson.id);
      const model = dungeonModel(7, challengesOf(7), lessonIds);
      const api = buildDungeonWorld(scene, model, model.theme);
      const art = api.worldArt;
      expect(art).toBe(scene.userData.worldArt);
      expect(art.palette).toBe(TAPEOUT_PALETTE);
      expect(art.landmark.userData.landmark).toBe(true);
      expect(art.landmark.position.x).toBe(ALTAR.x);
      expect(art.landmark.position.z).toBe(ALTAR.z);
