...
import { FOUNDRY_PALETTE, STACK } from '../src/graphics/foundry-world.js';
...
describe('Silicon Gothic Module Foundry', () => {
  test('composes an industrial bay with PBR steel, heat, and unshadowed point lights', () => {
    const restore = stubDocument();
    try {
      const scene = new THREE.Scene();
      const lessonIds = (LESSONS[3] || []).map(lesson => lesson.id);
      const model = dungeonModel(3, challengesOf(3), lessonIds);
      const api = buildDungeonWorld(scene, model, model.theme);
      const art = api.worldArt;
      expect(art).toBe(scene.userData.worldArt);
      expect(art.palette).toBe(FOUNDRY_PALETTE);
      expect(art.landmark.userData.landmark).toBe(true);
      expect(art.landmark.position.x).toBe(STACK.x);
      expect(art.landmark.position.z).toBe(STACK.z);
      expect(art.landmark.position.z).toBeGreaterThan(model.bounds.minZ);
      ...
      scene.traverse(object => {
        ...
        materials.forEach(material => {
          if (material?.userData?.surface === 'wornSteel') wornSteel += 1;
        });
      ...
      expect(wornSteel).toBeGreaterThan(3);
