...
import { CLOCK_PALETTE, CROWN } from '../src/graphics/clock-world.js';
...
describe('Silicon Gothic Clock Tower', () => {
  test('composes a brass interior with PBR, path lighting, and unshadowed point lights', () => {
    const restore = stubDocument();
    try {
      const scene = new THREE.Scene();
      const lessonIds = (LESSONS[5] || []).map(lesson => lesson.id);
      const model = dungeonModel(5, challengesOf(5), lessonIds);
      const api = buildDungeonWorld(scene, model, model.theme);
      const art = api.worldArt;
      expect(art).toBe(scene.userData.worldArt);
      expect(art.palette).toBe(CLOCK_PALETTE);
      expect(art.landmark.userData.landmark).toBe(true);
      expect(art.landmark.position.x).toBe(CROWN.x);
      expect(art.landmark.position.z).toBe(CROWN.z);
